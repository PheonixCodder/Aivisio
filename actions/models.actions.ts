...
export async function fetchModels(){
    ...
    return { error: error?.message || null, success: !error, data: data || null, count: count || 0 }
}
...
export async function deleteModel(id: number, model_id: string, model_version: string){
    ...
    if (model_version){
        try {
            const res = await fetch(`https://api.replicate.com/v1/models/pheonixcodder/${model_id}/versions/${model_version}`, { method: "DELETE", headers: { "Authorization": `Bearer ${process.env.REPLICATE_API_TOKEN}` } });
            if(!res.ok){
                throw new Error("Error deleting model version");
            }
        } catch (error) {
            console.log("Failed to delete model version", error);
    ...
    if (model_id){
        try {
            const res = await fetch(`https://api.replicate.com/v1/models/pheonixcodder/${model_id}`, { method: "DELETE", headers: { "Authorization": `Bearer ${process.env.REPLICATE_API_TOKEN}` } });
            if(!res.ok){
