...
const renderPricingBtn = ({subscription, user, product, price, MostPopular, handleStripePortalRequest, handleStripeCheckout}: {
  subscription: SubscriptionWithProduct | null
  user: User | null
  product: ProductWithPrices
  price: Price
  MostPopular: string
  handleStripePortalRequest: (product: ProductWithPrices) => Promise<void>
  handleStripeCheckout: (product: ProductWithPrices) => Promise<void>
}) => {
  if (user && !subscription){
    return (
      <Button variant={product.name?.toLowerCase() === MostPopular.toLowerCase() ? "default" : "outline"} className="mt-8 w-full font-semibold" onClick={() => handleStripeCheckout(price)}>Subscribe</Button>
    )
  }
}

const Pricing = ({ products, MostPopular = "pro", subscription, user }: PricingProps) => {
  ...
  return (
    <section className="max-w-7xl mx-auto px-8 py-16 w-full flex flex-col">
            ...
            return (
              ...
              >
                <div className="p-6">
                  ...
                  {
                    renderPricingBtn({subscription, user, product, price, MostPopular, handleStripeCheckout, handleStripePortalRequest})
                  }
                </div>
              </div>
            );
